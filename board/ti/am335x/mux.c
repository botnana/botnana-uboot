...
static struct module_pin_mux mapacode_gpio_pin_mux[] = {
	{OFFSET(mii1_rxdv), (MODE(7))},	/* GPIO3_4 */
	{OFFSET(mii1_rxclk), (MODE(7))},	/* GPIO3_10 */
	{OFFSET(gpmc_csn0), (MODE(7) | PULLUDDIS)},	/* GPIO1_29 */
	{OFFSET(lcd_vsync), (MODE(7))},	/* GPIO2_22 */

	{-1},
};

#if 1 /* added by chris(mapacode) */
static struct module_pin_mux rmii1_pin_mux[] = {
	{OFFSET(mii1_txen), MODE(1)},			/* RMII1_TXEN */
	{OFFSET(mii1_txd1), MODE(1)},			/* RMII1_TD1 */
	{OFFSET(mii1_txd0), MODE(1)},			/* RMII1_TD0 */
	{OFFSET(mii1_rxd1), MODE(1) | RXACTIVE | PULLUDDIS},	/* RMII1_RD1 */
	{OFFSET(mii1_rxd0), MODE(1) | RXACTIVE | PULLUDDIS},	/* RMII1_RD0 */
	{OFFSET(mii1_crs), MODE(1) | RXACTIVE | PULLUDDIS},		/* RMII1_CRS_DV */
	{OFFSET(mii1_rxerr), MODE(1) | RXACTIVE | PULLUDDIS},	/* RMII1_RXERR */
	{OFFSET(rmii1_refclk), MODE(0) | RXACTIVE | PULLUP_EN},	/* RMII1_refclk */
	
	{OFFSET(gpmc_a0), MODE(3)},			/* RMII2_TXEN */
	{OFFSET(gpmc_a4), MODE(3)},			/* RMII2_TD1 */
	{OFFSET(gpmc_a5), MODE(3)},			/* RMII2_TD0 */
	{OFFSET(gpmc_a10), MODE(3) | RXACTIVE | PULLUDDIS},	/* RMII2_RD1 */
	{OFFSET(gpmc_a11), MODE(3) | RXACTIVE | PULLUDDIS},	/* RMII2_RD0 */
	{OFFSET(gpmc_wait0), MODE(3) | RXACTIVE | PULLUDDIS},		/* RMII2_CRS_DV */
	{OFFSET(gpmc_wpn), MODE(3) | RXACTIVE | PULLUDDIS},	/* RMII2_RXERR */
	{OFFSET(mii1_col), MODE(1) | RXACTIVE | PULLUP_EN},			/* RMII2_refclk */
	
	{OFFSET(mdio_data), MODE(0) | RXACTIVE}, /* MDIO_DATA */
	{OFFSET(mdio_clk), MODE(0)},	/* MDIO_CLK */	
	{-1},
};
#endif
...
void enable_board_pin_mux(struct am335x_baseboard_id *header)
{
	...
	if (board_is_bone(header)) {
		...
		configure_module_pin_mux(mmc0_pin_mux);
		configure_module_pin_mux(mmc1_pin_mux);		
	} else if (board_is_mapabone(header)) {			/* added by chris(mapacode)  2015/01/20 */
		/* MapaBone pinmux */
		configure_module_pin_mux(rmii1_pin_mux);
		configure_module_pin_mux(mmc0_pin_mux);
		configure_module_pin_mux(mmc1_pin_mux);	

        configure_module_pin_mux(mapacode_gpio_pin_mux);
        if(gpio_request(100, "") == 0){
		    gpio_direction_output(100, 1);
        }
        else
            puts("...... gpio 100 be using!\n");

        if(gpio_request(86, "") == 0){
		    gpio_direction_output(86, 1);
        }
        else
            puts("...... gpio 86 be using!\n");
        
        if(gpio_request(61, "") == 0){
		    gpio_direction_output(61, 1);
        }
        else
            puts("...... gpio 61 be using!\n");

        if(gpio_request(106, "") == 0){
		    gpio_direction_output(106, 1);
        }
        else
            puts("...... gpio 106 be using!\n");
